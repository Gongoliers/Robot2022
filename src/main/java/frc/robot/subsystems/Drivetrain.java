...
public class Drivetrain extends SubsystemBase {
  ...
  private final MotorControllerGroup mLeftMotors =
      new MotorControllerGroup(mLeftMotor_1, mLeftMotor_2, mLeftMotor_3);
  ...
  private final MotorControllerGroup mRightMotors =
      new MotorControllerGroup(mRightMotor_1, mRightMotor_2, mRightMotor_3);
  ...
  public Drivetrain(VisionSubsystem vision) {
    ...
    var leftAverageEncoder =
        new AverageEncoderSensor(
    ...
    mRightEncoderSensor =
        rightAverageEncoder.scaledBy(DriveConstants.kInchesPerEncoderPulse).inverted();
    ...
    mPowerEfficiencyModule =
        new PowerEfficiencyModule(
            DriveConstants.kSecondsToReachFullSpeed, DriveConstants.kTurnThreshold);

    // TODO: Calibrate this
    mTargetAlignmentModule =
        new TargetAlignmentModule(
            vision.getTargetingCamera(), new PID(0.12, 0.05, 0.005), new PID(0, 0, 0), false);

    mModularDrivetrain.setModules(
        mTargetAlignmentModule, mVoltageControlModule, mPowerEfficiencyModule);
  }
  ...
  public DifferentialDriveWheelSpeeds getSpeeds() {
    return new DifferentialDriveWheelSpeeds(
        ...
        Units.feetToMeters(mLeftEncoderSensor.getVelocity())
            * DriveConstants.kGearRatio
            * 2
            * Math.PI
            * Units.inchesToMeters(DriveConstants.kWheelRadius)
            / 60,
        Units.feetToMeters(mRightEncoderSensor.getVelocity())
            * DriveConstants.kGearRatio
            * 2
            * Math.PI
            * Units.inchesToMeters(DriveConstants.kWheelRadius)
