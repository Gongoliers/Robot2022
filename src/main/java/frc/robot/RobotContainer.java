...
public class RobotContainer {
  ...
  private void configureAutonomous() {
    ...
    autoChooser.addOption(
        "Shoot Low then Leave Tarmac", new ShootLowThenLeaveTarmac(mDrivetrain, mShooter));
    autoChooser.addOption(
        "Shoot High then Leave Tarmac",
        new BackupAndShootHighThenLeaveTarmac(mDrivetrain, mShooter));
    autoChooser.addOption(
        "Leave Tarmac Shoot High", new LeaveTarmacAndShoot(mDrivetrain, mShooter, m_intake));
    autoChooser.addOption(
        "Leave Tarmac Shoot Low", new LeaveTarmacAndShootLow(mDrivetrain, mShooter, m_intake));
  ...
  public void quickRumble(boolean left) {
    ...
    timer.schedule(
        new TimerTask() {
          @Override
          public void run() {
            m_manipulatorController.setRumble(
                left ? RumbleType.kLeftRumble : RumbleType.kRightRumble, 0);
          }
        },
        500);
  }

  /** END OF FRC CODE THE FOLLOWING CODE IS SOLELY DEDICATED TO MAPPING BINDINGS */

  /** SECTION 1: -- Configuring The Driver's Bindings */
  private void configureDriverBindings() {
    // Driver Joystick Buttons
    m_driverJoystick = new Joystick(OIConstants.kDriverControllerPort);

    /**
     * Turbo Button Binding -- Enables / Disables Turbo Functionality
     *
     * <p>Binding: -- Joystick Trigger
     */
    ...
    /**
     * Driver's Joystick Axis -- Allows the driver to control the robot
     *
     * <p>Binding: -- Y (Axis 2) Controls speed +/- -- Z (Axis 3) Controls rotation
     */
    mDrivetrain.setDefaultCommand(new DrivetrainOperatorControl(this, mDrivetrain));

    /**
     * Driver's Stop All Command -- Stops all Active Subsystems on the Robot
     *
     * <p>Binding: -- Button 4 (bottom left button @ top of joystick)
     */
    ...
    /**
     * Switch Left & Right Directions -- Switches Left & Right Directions -- Requested by Driveteam
     *
     * <p>Binding: -- Button 2 (button on side of axis)
     */
    ...
    /**
     * Backup for High Goal -- Backs Up from Fender to Shoot for high goal
     *
     * <p>Binding: -- Button 10
     */
    // TODO: Use this
    var backup =
        new SimpleDriveDistance(mDrivetrain, AutoConstants.kDistanceToDriveForHigh, 0.05, 0.3)
            .withTimeout(3.0);
    JoystickButton positionForHigh = new JoystickButton(m_driverJoystick, 10);
    positionForHigh.whenPressed(backup);

    /**
     * Override Match Timer -- Overrides Match Timer Lock for Endgaem -- Reevaluate if this is
     * necessary
     *
     * <p>Binding: -- Button 11
     */
    ...
    /**
     * Backup and Shoot -- Backups and Shoot
     *
     * <p>Binding: -- Button ??
     */
    ...
    manipulatorBackupFootandShoot.whenReleased(
        new ParallelCommandGroup(new StopDrivetrain(mDrivetrain), new StopShooter(mShooter)));

    /**
     * Shoot High -- Shoots High
     *
     * <p>Binding: -- Button ???
     */
  ...
  private void configureManipulatorBindings() {
    ...
    /**
     * Start Compressor Command -- Starts the Compressor (for Pneumatics)
     *
     * <p>Binding: -- X button
     */
    JoystickButton startCompressor =
    ...
    /**
     * Stop Compressor Command -- Stops the Compressor (for Pneumatics)
     *
     * <p>Binding: -- B button
     */
    JoystickButton stopCompressor =
    ...
    /**
     * Raise Endgame -- Raises Endgame
     *
     * <p>Binding: -- Up DPAD Button
     */
    ...
    /**
     * Lower Endgame -- Lowers Endgame
     *
     * <p>Binding: -- Down DPAD Button
     */
    ...
    /**
     * Break Safety -- Disables timer safety -- Re-add if necessary
     *
     * <p>Binding: -- Right DPAD Button
     */
    // Trigger endgameEnabled = new Trigger(EndgameTimer::isTimerBroken).negate();
    // Trigger overrideEndgame = new DPadButton(m_manipulatorController,
    // Direction.RIGHT).and(endgameEnabled);
    // overrideEndgame.whenActive(new OverrideEndgame());

    // Trigger endgameDisabled = new Trigger(EndgameTimer::isTimerBroken);
    // Trigger overrideEncoders = new DPadButton(m_manipulatorController,
    // Direction.RIGHT).and(endgameDisabled);
    // overrideEncoders.whenActive(new DisableEncoderChecks(m_endgame));

    // Shooter Subsystem

    /**
     * Shoot Balls -- Shoots Balls
     *
     * <p>Binding: -- Right Trigger
     */
    Button shootBalls =
        Hardware.makeButton(
            new BooleanSupplier() {
    ...
    /**
     * Shoot Balls Low -- Shoots Balls
     *
     * <p>Binding: -- Right Bumper
     */
    JoystickButton shootBallsLow =
        new JoystickButton(m_manipulatorController, XboxController.Button.kRightBumper.value);
    shootBallsLow.whileHeld(new ShootLow(mShooter));

    // Intake Subsystem

    /**
     * Deploy Intake -- Deploys the Intake
     *
     * <p>Binding: -- Y
     */
    ...
    /**
     * Retract Intake -- Retracts the Intake
     *
     * <p>Binding: -- A
     */
    JoystickButton retractIntake =
    ...
    /**
     * Activate Intake -- Activates the Intake
     *
     * <p>Binding: -- Left Trigger
     */
    Button intakeBalls =
        Hardware.makeButton(
            new BooleanSupplier() {
    ...
    /**
     * Outtake -- Outtakes Any Balls
     *
     * <p>Binding: -- Left Bumper
     */
    JoystickButton outtakeBalls =
        new JoystickButton(m_manipulatorController, XboxController.Button.kLeftBumper.value);
